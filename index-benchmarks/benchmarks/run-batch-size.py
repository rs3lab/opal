...
BATCH_SIZE = [16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384, 32768, 65536]
SECONDS = 10
SERVER = 'srv9'
NUM_REPLICATES = 5

if SERVER == 'srv1':
    threads = [1, 2, 4, 8, 16, 28, 56, 84, 112, 128, 168, 224]
...
base_data_dir = os.path.join(base_repo_dir, 'plots', 'index', 'nopinning-data-batch_size')
...
class PiBenchExperiment:
    ...
    def run(self, ith, idx, total):
        ...
        result_text = None
        skipped = False
        if os.path.exists(os.path.join(data_dir, f'{self.name}.raw', f'{self.index}-{self.kwargs["threads"]}-{self.kwargs["batch_size"]}-{ith}.out')):
            skipped = True
            print('Skipping')
        result = None
        if not skipped:
            ...
            with open(os.path.join(data_dir, f'{self.name}.raw', f'{self.index}-{self.kwargs["threads"]}-{self.kwargs["batch_size"]}-{ith}.out'), 'r') as f:
                result_text = f.read()
        ...
        if not skipped:
            with open(os.path.join(data_dir, f'{self.name}.raw', f'{self.index}-{self.kwargs["threads"]}-{self.kwargs["batch_size"]}-{ith}.out'), 'w') as f:
                f.writelines(result.stdout)


gl_df_columns = ['exp', 'name'] + [f'{opType}-ratio' for opType in PiBenchExperiment.opTypes] + ['key-type', 'distribution', 'skew-factor', 'batch_size'] + ['index', 'thread', 'replicate'] + PiBenchExperiment.finishTypes + [
...
def run_all_experiments(expname, name, indexes, labels, threads, dense=True, *args, **kwargs):
    ...
    df['batch_size'] = kwargs['batch_size']
    ...
    for num_record,record_label in zip(NUM_RECORDS, NUM_RECORD_LABELS): 
        for batch_size in BATCH_SIZE:
            ...
            run_all_experiments('scalability', 'Update-only-selfsimilar', indexes, labels, threads, records=num_record, seconds=SECONDS,
                                read_ratio=0.0, update_ratio=1.0, distribution='SELFSIMILAR', skew=skew, batch_size=batch_size)
