...
bool Tree::update(Key &k, TID new_tid) {
restart:
  bool needRestart = false;
  ...
  while (true) {
    switch (checkPrefix(node, k, level)) {  // increases level
      ...
      case CheckPrefixResult::Match:
        ...
        if (N::isLeaf(node)) {
          ...
          exit(4); // Code should not reach here
        }
        level++;
    }
    uint64_t nv = OMCSLock::kInvalidVersion;
    if (level == k.getKeyLen() - 1) {
      ...
      // [node] is at the last level of the ART that corresponds to an ART_OLC::N
      // I should lock [node] exclusively
      int res = node->executeOp(&k, new_tid, node, parentNode, v);
      if(res == -1)
        goto restart;
      return res;
    } else {
...
inline typename ART_OLC::CheckPrefixResult Tree::checkPrefix(N *n, const Key &k, uint32_t &level) {
  if (n->hasPrefix()) {
