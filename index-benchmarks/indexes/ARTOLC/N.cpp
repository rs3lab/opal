...
int N::updateOperation(void *kptr, uint64_t new_tid, void *nodeptr, void *parentNodeptr, uint64_t v) {
  bool needRestart = false;
  bool optimisticPrefixMatch = false;

  auto node = static_cast<N*>(nodeptr);

  node->checkObsoleteOrRestart(needRestart);
  if(needRestart) return -1;

  auto k = static_cast<Key*>(kptr);
  auto parentNode = static_cast<N*>(parentNodeptr);

  parentNode->readUnlockOrRestart(v, needRestart);
  if(needRestart) return -1;

  auto level = k->getKeyLen() - 1;

  auto child_node = N::getChild((*k)[level], node);

  if (child_node == nullptr) return false;

  if (N::isLeaf(child_node)) {
    N *tid = N::setLeaf(new_tid);
    N::change(node, (*k)[level], tid);
    return true;
  }
  exit(1); //Code should not reach here
}
#endif
