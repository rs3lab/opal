...
class N: public OMCSLock {
 ...
 public:
  ...
  void upgradeToWriteLockOrRestart(uint64_t &version, bool &needRestart) {
    OMCSLock::upgradeToWriteLockOrRestart(version, needRestart);
    if (needRestart) return;
    checkObsoleteOrRestart(needRestart);
    if (needRestart) {
      OMCSLock::writeUnlock();
    }
  }
  ...
  void writeLockOrRestart(Context *q, bool &needRestart) {
    OMCSLock::writeLock(q);
    checkObsoleteOrRestart(needRestart);
    if (needRestart) {
      OMCSLock::writeUnlock(q);
    }
  }

  void upgradeToWriteLockOrRestart(uint64_t &version, Context *q, bool &needRestart) {
    OMCSLock::upgradeToWriteLockOrRestart(version, q, needRestart);
    if (needRestart) return;
    checkObsoleteOrRestart(needRestart);
    if (needRestart) {
      OMCSLock::writeUnlock(q);
  ...
  uint64_t readLockOrRestart(bool &needRestart) const {
    auto v = OMCSLock::readLockOrRestart(needRestart);
    checkObsoleteOrRestart(needRestart);
    return v;
  }

  void checkOrRestart(uint64_t startRead, bool &needRestart) const {
    OMCSLock::checkOrRestart(startRead, needRestart);
    checkObsoleteOrRestart(needRestart);
  }

  void readUnlockOrRestart(uint64_t startRead, bool &needRestart) const {
    OMCSLock::readUnlockOrRestart(startRead, needRestart);
    checkObsoleteOrRestart(needRestart);
  }

#if defined(ART_OFP_LOCK)
  static int updateOperation(void *k, uint64_t new_tid, void *node, void *parentNode, uint64_t v);

  int executeOp(void *k, uint64_t new_tid, void* node, void *parentNode, uint64_t v) {
    return executeARTOp(k, new_tid, node, parentNode, v, &updateOperation);
  }
#endif
