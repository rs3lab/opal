...
LOCK_DEFINE_TIMING_VAR(tree_traversal_for_reads);
...
#ifdef RETRY_INSERT_COUNT
constexpr int kMaxInsertRetries = 1;
#else
...
struct NodeBase : public OMCSLock {
  ...
  virtual ~NodeBase() = default; // Virtual destructor
};
...
//#if defined(OFP_LOCK)
//  bool update(uint64_t k, uint64_t p) {
//    assert(count <= maxEntries);
//    if (count) {
//      unsigned pos = lowerBound(k);
//      if ((pos < count) && (data[pos].first == k)) {
...
struct BTreeBase {
  ...
  bool lookup(Key k, Value &result) {
    LOCK_START_TIMING(tree_traversal_for_reads);
    int restartCount = 0;
  restart:
    ...
    while (node->getType() == PageType::BTreeInner) {
#if defined(MUTEX_OPTIMIZATION)
			//#include "latches/OpFPMutex.h"
			//volatile int *addr = &(komb_base_qnode[thread_to_node_mapping[komb_cur_cpu_id]].is_combiner_running);
			//while(*addr == 1) {
			//	sys_futex((int *)addr, FUTEX_WAIT_PRIVATE, 1, NULL, 0, 0);
			//}	
#endif
      auto inner = static_cast<BTreeInner<Key> *>(node);
    ...
    ADD_TO_STATS_WITHOUT_DIST(traversal_for_read_restart, (restartCount-1));
    LOCK_END_TIMING(tree_traversal_for_reads);
    return success;
  }
#endif
