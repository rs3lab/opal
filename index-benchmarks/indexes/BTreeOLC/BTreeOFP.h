...
// This implementation uses OMCS only on leaf nodes with function pointer for callback.
...
struct BTreeOFP : public BTreeBase<Key, Value> {
  ...
  BTreeOFP() {
    std::cout << "========================================" << std::endl;
    std::cout << "BTree with OMCS on leaf nodes with function pointer " << std::endl;
      ...
      for (size_t i = top; i; i--) {
        ...
        if (needRestart) {
          for (size_t j = i; j < top; j++) {
            //nodes[j].first->writeLockTurnOffOpRead(); //TODO: Merge this
  ...
  restart:
    ...
    while (node->getType() == PageType::BTreeInner) {
      ...
      if (next->getType() == PageType::BTreeInner) {
        ...
        // [next] is a leaf node
        next->writeLock(&q); //TODO: optimize this to enable op read
    ...
    if (leaf->isFull()) {
      ...
      if (read_nodes.Size() == 0) {
        ...
        while (read_nodes.Size() > 1) {
          ...
          //n->writeLockTurnOffOpRead(); //TODO: check this
    ...
    ADD_TO_STATS_WITHOUT_DIST(traversal_for_insert_restart, (restartCount -1));
    return ok;
  }
  ...
  bool traverseToLeafEx(Key k, OMCSLock::Context &q, NodeBase *&node, uint64_t &versionNode) {
    //LOCK_START_TIMING_PER_CPU(traverse_leaf);
    int restartCount = 0;
  restart:
    if (restartCount++) yield(restartCount);
    bool needRestart = false;
    ...
    while (node->getType() == PageType::BTreeInner) {
      ...
      if (next->getType() == PageType::BTreeInner) {
        ...
        //LOCK_END_TIMING_PER_CPU(traverse_leaf);
...
exclusive_latch:
    ADD_TO_STATS_WITHOUT_DIST(traversal_for_write_restart, (restartCount-1));
  ...
  restart:
    ...
    versionNode = OMCSLock::kInvalidVersion;
    node = root;
    assert(node->getType() != PageType::BTreeLeaf);
    versionNode = node->readLockOrRestart(needRestart);
    if (needRestart || node != root) goto restart;
    ...
    while (node->getType() == PageType::BTreeInner) {
      ...
      if (next->getType() == PageType::BTreeInner) {
        ...
        int res = next->executeOperation(node, versionNode, next, k, v);
        if(res == -1)
          goto restart;
        else {
          ADD_TO_STATS_WITHOUT_DIST(traversal_for_write_restart, (restartCount-1));
