...
LOCK_DEFINE_TIMING_VAR(tree_traversal_for_writes);
...
struct BTreeLC : public BTreeBase<Key, Value> {
  ...
  restart:
    ...
    if (node != root) {
      ...
      // fprintf(stderr, "node was updated? %p\n", (void*) node);
    ...
    if (leaf->isFull()) {
      ...
      #ifdef TC_LOCK
        // fprintf(stderr, "inserting into LEAF %p\n", (void*) node->lock.lock_);
      #endif
      // no need to split, just insert into [leaf]
      assert(latched_nodes.Size() == 1);
      bool ok = leaf->insert(k, v);
      node->writeUnlock(&q);
      // fprintf(stderr, "insert into LEAF ok %p\n", (void*) node->lock.lock_);
  ...
  bool traverseToLeaf(Key k, OMCSLock::Context &q, NodeBase *&node) {
    if constexpr (ty == Sh) {
      ...
      LOCK_START_TIMING(tree_traversal_for_writes);
    }
    int restartCount = 0;
  restart:
    if (restartCount++) yield(restartCount);
    ...
    while (node->getType() == PageType::BTreeInner) {
      ...
      if (next->getType() == PageType::BTreeInner) {
        ...
        if constexpr (ty == Sh) {
          ...
          LOCK_END_TIMING(tree_traversal_for_writes);
          next->writeLock(&q);
        }
      }
      node->readUnlock(&q);
