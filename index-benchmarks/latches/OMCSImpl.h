...
class OMCSLock {
 public:
#ifdef OMCS_OFFSET
  static constexpr uint64_t kQueueNodeIdBits = 12;  // At most 1024 qnodes
  static constexpr uint64_t kNumQueueNodes = 1 << kQueueNodeIdBits;
  static constexpr uint64_t kVersionBits = 50;
...
#ifdef OMCS_OFFSET
  static constexpr uint64_t kVersionMask = 0x7ffffffffffff;
#ifdef OMCS_OP_READ
 ...
 public:
  ...
  uint64_t lock() {
    ...
    while (true) {
      ...
      // lock acquired
      // LOCK_START_TIMING_PER_CPU(write_critical_section);
      return version;
    }
  }
  ...
  bool try_lock(uint64_t version) {
    ...
    uint64_t curr = version;
    uint64_t locked = OMCSLock::make_locked_version(version);
    bool res = tail_.compare_exchange_strong(curr, locked);
    if (res) {
      // LOCK_END_TIMING_PER_CPU(opt_readers);
    }
    return res;
  }

  void unlock() {
    // LOCK_END_TIMING_PER_CPU(write_critical_section);
...
//     unlock_release = qnode->release_time;
// #endif
//     LOCK_END_TIMING_PER_CPU(unlock_release);
  }
  ...
  void unlock(OMCSQNode *qnode) {
    ...
    if (succ == nullptr) {
      ...
      // LOCK_START_TIMING_PER_CPU(unlock_release);
...
#endif  // OMCS_OFFSET
      // LOCK_START_TIMING_PER_CPU(opt_readers);
#endif  // OMCS_OP_READ
...
// #ifdef LOCK_MEASURE_TIME
//     succ->release_time = unlock_release;
// #endif
    succ->set_version(v1);
  }
